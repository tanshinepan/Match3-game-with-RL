...
class game_matrix:
    __slots__ = "edit_cubes", "round", "color_matrix", "new_color", "sand_board", "cube_count", "cube_matrix"
    def __init__(self):
        self.cube_matrix = [[0 for j in range(NUMOFCUBE)] for i in range(NUMOFCUBE)]
        self.edit_cubes = ()
        self.cube_count = 0
        self.sand_board = np.array([[NUMOFCUBE for i in range(LEVEL)] for j in range(LEVEL)])
        ...
        while True:
            while True:
                ...
                if np.count_nonzero(self.sand_board == NUMOFCUBE) == 0:
                    break
            self.match_once((), 2)
    ...
    def remove_high_level(self):
        self.sand_board = np.where(self.sand_board > 10, NUMOFCUBE, self.sand_board)

    def get_random_cube(self):
        return choice(list(range(0,NUMOFCUBE)))

    def get_next_cube(self):
        self.cube_count += 1
        return randint(0,NUMOFCUBE-1)

    def get_new_cubes(self,data,random):
        # 补充第一排
        if random:
            for column in range(LEVEL):
                if data[0][column] == NUMOFCUBE:
                    data[0][column] = self.get_random_cube()
                elif data[0][column] < 0:
                    for i in range(1, LEVEL):
                        if data[i][column] == NUMOFCUBE:
            ...
            # 统计每列的空格数
            blank_list = np.array(
                [np.count_nonzero(data[:, i] == NUMOFCUBE) for i in range(LEVEL)])
            # 统计每列的砖块数
    ...
    def get_sand_board_init(self):
        ...
        temp[temp < 0] = temp[temp < 0] * 10 + NUMOFCUBE
    ...
    def move_down(self, data):
        ...
        for column in range(LEVEL):
            ...
            for i in range(LEVEL - 1):
                row_i = LEVEL - i - 1
                # 如果下面的块不是空的，就pass
                if data[row_i][column] != NUMOFCUBE:
                    continue
                # 如果是空的，就找最下面的那个空当
                for j in range(i, LEVEL):
                    row_j = LEVEL - j - 1
                    if data[row_j][column] != NUMOFCUBE and data[row_j][column] >= 0:
    ...
    def has_pair(self, row, column, data, dir):
        if data[row, column] == NUMOFCUBE or data[row, column] < 0:
            return False
        if dir == "u":
            if data[row, column] == data[row - 1, column] == data[row - 2, column]:
                return True
        elif dir == "ul":
            if data[row, column] == data[row - 1, column - 1] == data[row - 2, column - 2]:
                return True
        elif dir == "ur":
            if data[row, column] == data[row - 1, column + 1] == data[row - 2, column + 2]:
                return True
        elif dir == "l":
            if data[row, column] == data[row, column - 1] == data[row, column - 2]:
                return True
        else:
